...
void bossfire(){
	...
	{
		if (myscore<1)
			f[i].laser_show = false;

		if (b[i].boss_show)
		{
			iShowBMP(f[i].laser_x, f[i].laser_y, "boss\\laser1.bmp");
		}
	}
	for (int i = 0; i < 1; i++)
	{
		if (f[i].laser_y < 0)
		{
			f[i].laser_x = b[i].boss_x + 75;
			f[i].laser_y = b[i].boss_y - 40;
			f[i].laser_show = true;
		}
	}
}
...
void killboss(){
	for (int i = 0; i < 30; i++)
	{
	  if (collision(b[0].boss_x, b[0].boss_y, 150, 100, fireBall[i].fire_x, fireBall[i].fire_y, 12, 20))
	   {
	    fireBall[i].fire_OnOff = false;
		fireBall[i].fire_x = ((length / 2) - 5) + fx;
		fireBall[i].fire_y = SHIPHEIGHT + 60;
...
void iKeyboard(unsigned char key)
{
	...
	if (key == ' ' && start == 1)
	{
		...
		fireBall[fireIndex].fire_y = SHIPHEIGHT + 60;
		++fireIndex;
	}
	//select option (menu)
